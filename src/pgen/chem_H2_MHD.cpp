...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  const Real vy = pin->GetOrAddReal("problem", "vy", 0); //velocity y
  const Real vz = pin->GetOrAddReal("problem", "vz", 0); //velocity z
  const Real b0 = pin->GetOrAddReal("problem", "b0", 0); // Mean Field Strength
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        Real     T  =  std::max(E_ergs / (1.5*1.381e-16),Tfloor);
        Real Heating = 2e-26;
...
void MeshBlock::UserWorkInLoop(){
  //Clean the bad temperature pixel
  const Real unit_length_in_cm_  = 3.086e+18;
  const Real unit_vel_in_cms_    = 1.0e5;
  const Real unit_density_in_nH_ = 1;
  const Real unit_time_in_s_ = unit_length_in_cm_/unit_vel_in_cms_;
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        // set a temperature floor if T < T_floor
        Real   nH_  = phydro->u(IDN,k,j,i)*unit_density_in_nH_;
        Real   ED   = phydro->w(IPR,k,j,i)/(g - 1.0);
        Real E_ergs = ED * unit_E_in_cgs_ / nH_; // unit : erg cm^{-3}
        Real E_min  = Tfloor*(1.5*1.381e-16); // unit : erg cm^{-3}
        E_ergs  = std::max(E_ergs,E_min);
        phydro->w(IPR,k,j,i) = E_ergs*nH_/unit_E_in_cgs_*(g - 1.0);
      }
    }
  }
}
